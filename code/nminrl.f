...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      integer :: j, k, kk
      real*8 :: rmn1, rmp, xx, csf, rwn, hmn, hmp, r44, cnr, cnrf, cpr
...
!          sut = Min(1., sut)
     ...
     &                                                    sol_orgn(k,j))
          ...
          !! fresh organic n and p (upper two layers only) !!李玮翔把R4改成了R44
          rmn1 = 0.
          rmp = 0.
          if (k <= 2) then
            r44 = 0.
            ...
            if (sol_fon(k,j) + sol_no3(k,j) > 1.e-4) then
              cnr = 0.
              cnr = r44 / (sol_fon(k,j) + sol_no3(k,j))
              if (cnr > 500.) cnr = 500.
              cnrf = 0.
            ...
            if (sol_fop(k,j) + sol_solp(k,j) > 1.e-4) then
              cpr = 0.
              cpr = r44 / (sol_fop(k,j) + sol_solp(k,j))
