...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
	...
	real*8 :: sanloss, silloss, claloss, sagloss, lagloss
      ...
      real*8 :: temp_pot1, temp_pot2, temp_pot3
      temp_pot1 = 0
      temp_pot2 = 0
      temp_pot3 = 0
      j = 0
...
!        calculate seepage into soil
         potsep = R4(j)                     !!mm/h*ha/240=m3/cnv=mm
         potsep = Min(potsep, pot_vol(j)) !!修正渗漏
         potvol_sep = pot_vol(j)
         pot_vol(j) = pot_vol(j) - potsep
         pot_seep(j) = potsep
!         sol_st_lwx(1,j) = sol_st(1,j)
