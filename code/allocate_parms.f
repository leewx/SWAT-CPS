...
!!    LID general
	...
	allocate(rspc_a(mhru))	
      ...
      allocate(pot_flag(mhru))
      !!lwx 2022-4-11 
      !!修改自李玮翔 2022-4-12 
      allocate (landuse_hru(mhru))
      allocate (pot_vol_sorted(mhru))
      allocate(landuse_hru_num(100))!!开大点，防超
      allocate(landuse_hru_avg(100))!!开大点，防超
      allocate(H_pot_min(mhru))
      allocate(H_pot_max(mhru))
      allocate(H_pot_avg(mhru))
      allocate (H_pot_max_idl(366))
      allocate (H_pot_min_idl(366))
      allocate(water_high_warn(1000000))
      allocate(pot_vol_temp(mhru))
      allocate(slope_hru(mhru))
      allocate(K_lwx(mhru))
      allocate(high_hru(mhru))
      allocate(water_high_c(1000000))!!防超
      allocate(lwx_theta_r(mhru))
      allocate(lwx_theta_s(mhru))
      allocate(lwx_alpha(mhru))
      allocate(lwx_npar(mhru))
      allocate(lwx_theta_r_data(mhru))
      allocate(lwx_theta_s_data(mhru))
      allocate(lwx_alpha_data(mhru))
      allocate(lwx_npar_data(mhru))
      allocate(lwx_n(mhru))
      allocate(lwx_m(mhru))
      allocate(lwx_se(mhru))
      allocate(lwx_theta(mhru))
      allocate(lwx_kr(mhru))
      allocate(K_lwx_nowater(mhru))
      allocate(soil_k_code(mhru))
      allocate(theta_sw(366,mhru))
      allocate(es_day_last(366,mhru))
      allocate(R4(mhru))
      allocate(R5(mhru))
      allocate(R7(mhru))
      allocate(Each_iihru_flow_go(mhru))
      allocate(Each_iihru_flow_accept(mhru))
      allocate(Each_iihru_no3_go(mhru))
      allocate(irr(mhru))
      allocate(sand_usle(mhru))
      allocate(sedorgn_seep(mhru))
      allocate(Each_iihru_orgn_go(mhru))
      !!2022-5-19
      call zero0
      call zero1
      call zero2
