...
!! By Zhang for C/N cycling
      integer, dimension(:), allocatable :: pot_flag
      !! lwx 2022-4-11
      integer, dimension (:), allocatable :: landuse_hru
      real*8, dimension(:), allocatable :: pot_vol_sorted
      integer, dimension (:), allocatable :: landuse_hru_num
      real*8, dimension (:), allocatable :: landuse_hru_avg
      real*8, dimension (:), allocatable :: H_pot_min
      real*8, dimension (:), allocatable :: H_pot_max
      real*8, dimension (:), allocatable :: H_pot_avg
      real*8, dimension (:), allocatable :: H_pot_max_idl
      real*8, dimension (:), allocatable :: H_pot_min_idl
      real*8, dimension (:), allocatable :: water_high_warn
      real*8, dimension (:), allocatable :: pot_vol_temp
      real*8, dimension (:), allocatable :: slope_hru
      real*8, dimension (:), allocatable :: K_lwx
      real*8, dimension (:), allocatable :: high_hru
      real*8, dimension (:), allocatable :: water_high_c
      real*8, dimension (:), allocatable :: irr
      integer :: days_pot
      integer :: count_fl
      integer, dimension(9574,9574):: matrix_hru
      integer, dimension(9574,9574) :: matrix_hru_slope !!通过达西公式 Q/A = K(H2-H1)/L   L 过水断面长，取土堤宽，K，根据不同土壤取水平流 Q(m3/d) A = sqrt(S_hru) * pot_vol(hrui) pot_vol以slope大的为准
                                                        !! 这个公式整体转化为 Q = sqrt(S_hru) * pot_vol(hrui)*K*(pot_vol(ihru)*pot_vol(jhru))/L   i为上游slope大的  L为定长,K应分类讨论
      real*8 :: H_lwx_1,H_lwx_2,H_lwx_3,H_lwx_4,H_lwx_ii,H_lwx_jj
      real*8 :: Lpot
      !!!!!修改自李玮翔 2022-4-17
      real*8, dimension (:), allocatable :: lwx_theta_r, lwx_theta_s, lwx_alpha, lwx_npar
      real*8, dimension (:), allocatable :: lwx_theta_r_data, lwx_theta_s_data, lwx_alpha_data, lwx_npar_data
      real*8, dimension (:), allocatable :: lwx_n, lwx_se, lwx_theta, lwx_kr, lwx_m, K_lwx_nowater, soil_k_code
      real*8, dimension (:,:), allocatable :: theta_sw
      real*8, dimension (:,:), allocatable :: es_day_last
      real*8, dimension (:), allocatable :: R4
      real*8, dimension (:), allocatable :: R5
      real*8, dimension (:), allocatable :: R7
      real*8, dimension (:), allocatable :: Each_iihru_flow_go,Each_iihru_flow_accept,Each_iihru_no3_go
      real*8, dimension (:), allocatable :: sand_usle,sedorgn_seep,Each_iihru_orgn_go
